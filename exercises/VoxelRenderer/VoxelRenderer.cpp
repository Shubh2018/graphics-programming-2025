...
VoxelRenderer::VoxelRenderer(int voxelCountX, int voxelCountZ, glm::vec4 lightColor, glm::vec3 lightPosition, float lacunarity = 1.9f, float gain = 0.5f, float octaves = 8.f, float modifiers = 10.f)
{
...
void VoxelRenderer::GenerateVoxelList()
{
	...
	for (int x = 0; x < m_voxelCountX; x++)
	{
		for (int z = 0; z < m_voxelCountZ; z++)
		{
			...
			//
			positions.push_back(glm::ivec3(static_cast<GLint>(x), static_cast<GLint>(abs(heights[x * m_voxelCountZ + z]) * m_modifier), static_cast<GLint>(z)));
...
std::vector<float> VoxelRenderer::CreateHeightMap(unsigned int countX, unsigned int countZ, glm::ivec2 coords)
{
	...
	for (unsigned int j = 0; j < countZ; ++j)
	{
		for (unsigned int i = 0; i < countX; ++i)
		{
			...
			pixels[j * countX + i] = stb_perlin_fbm_noise3(x, 0.f, y, m_lacunarity, m_gain, m_octaves);
		}
	}
